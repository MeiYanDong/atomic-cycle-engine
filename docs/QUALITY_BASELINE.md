...
| 要求           | 证据                                          | 当前结论                                                                                  |
| -------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------- |
| 高风险边界测试 | `test/*.test.ts`                              | 15/15：跨链/重复池、混合状态、成本后利润、只读 RPC、敏感证据、机会分母、来源归因          |
...
| 只读链上核验   | `docs/evidence/2026-09-11-phase0-readback.md` | Base 14/14、Robinhood 7/7 登记地址有 bytecode；只证明 code presence                       |

## 缺口、影响与优先级

| 优先级 | 缺口                                                         | 影响                                                 |
| ------ | ------------------------------------------------------------ | ---------------------------------------------------- |
| P0     | Factory/PoolManager gap-free backfill 未实现                 | 无法证明池全集与机会频率                             |
...
只实现 Phase 1 的前三张故事卡：Base P0 factory census、Robinhood 只读普查迁移、状态缓存/依赖唤醒。它们先回答“池子是否覆盖”和“机会是否真的存在”，再决定是否实现 live quote adapters 与是否购买 RPC。生产执行、CD 和钱包保持冻结。
