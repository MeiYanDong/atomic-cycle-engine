...
| 补充 `manga-chan-atomic-arbitrage/Earn` | `/Users/myandong/Projects/manga-chan-atomic-arbitrage/src/earnonhood-routes.mjs`                  | `adapt_then_reuse` | 复用 3 个已核验池、4 条 reviewed 路线、固定块 BatchRouter 报价和执行边界；不复制钱包、密钥、授权、服务配置或把混合路线说成已可执行。 |
...
- Earn/Balancer v3 池级报价、 reviewed 路线簿和“独立执行器存在不等于本服务已授权”的边界。
